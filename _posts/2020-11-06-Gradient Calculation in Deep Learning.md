...
## 1.1 深度学习中几种常见的梯度计算
在神经网络中，很常见的求梯度类型是一个**实值函数$f$**(如损失函数)对**一个向量$\bm{v}$**(如网络某一层的输出)或**一个矩阵$\bm{W}$**(如网络中的参数)求梯度，他们的实质其实都是多元函数求偏导。

### 1.1.1 常数(实值函数)对向量求梯度
...
由1.1.1和1.1.2可知，对向量或矩阵求梯度，就是对其中的每一个元素分别求偏导，所以我们开始吧。
