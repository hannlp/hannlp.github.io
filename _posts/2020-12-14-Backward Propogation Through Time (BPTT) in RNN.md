...
2. 每一时刻$\bm{t}$的**输入$\bm{x^{(t)}}$** 都是一个向量，在经过模型后会得到**这一时刻的状态$\bm{h^{(t)}}$** 和**输出$\bm{y^{(t)}}$**。
3. 在NLP中，$\bm{y^{(t)}}$是由$\bm{s^{(t)}}$经过$g$ (通常为Softmax) 激活得到的，搭配Cross Entropy Loss (比如在词表中挑选下一个单词，这是一个多分类问题) ，就能计算出此刻的损失$\bm{E^{(t)}}$。
4. 但是计算出$\bm{E^{(t)}}$后，并不能立即对模型参数进行更新。需要沿着时间