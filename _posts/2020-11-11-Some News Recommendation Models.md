...
> 
> 2. 再将$\bm{e_u}$经过一个非线性映射，得到一个查询向量$\bm{q_w}=ReLU(\bm{V_w}\times\bm{e_u}+\bm{v_w})$，其中$\bm{V_w}$和$\bm{v_m}$都是模型参数(就是会不断被更新的辣种)，下面同理!
> 
...
* **短期兴趣表示**：
作者使用了GRU(gated recurrent networks)来根据用户的浏览历史(按时间顺序)学习用户的短期兴趣表示。若新闻序列为$(\bm{e_1,e_2,...,e_k})$，有如下公式(即GRU的几个公式)：
